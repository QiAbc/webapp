...
export default function Detail({ params }: { params: { id: string } }) {
  ...
  const init = () => {
    createVirtualUser()
    setSubRatingVal(1)
    fetch(`/api/movies?id=${params.id}`).then(response => response.json())
      .then(data => setData(data[0]));

    fetch(`/api/movie_reviews?id=${params.id}`).then(response => response.json())
  ...
  const sub = async () => {
    ...
    const res = await fetch('/api/submit_reviews', {
      method: 'POST',
      headers: {
  ...
  return (
    data && <main className="flex flex-col items-center">
      ...
      <div className="flex gap-x-4 pt-8 text-sm">
        ...
        />
        <div>
          <div><span className='opacity-80'>Genre: </span>{data.genre}</div>
          <div><span className='opacity-80'>Year: </span>{data.year}</div>
          <div className="flex items-center"><span className='opacity-80'>Rating: </span>&nbsp; <StarRating disabled rating={data.rating} /> </div>
          <div><span className='opacity-80'>Classification: </span>{data.classification}</div>
          <div><span className='opacity-80'>Cast: </span>{data.cast}</div>
          <div><span className='opacity-80'>Director: </span>{data.director}</div>
        </div>
      </div>
        ...
        <div className='pt-8 '>
          ...
          <div className='pt-8 text-lg flex justify-between items-center'>
            ...
            <Button onPress={onOpen}>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" role="presentation"><path d="M18 13h-5v5c0 .55-.45 1-1 1s-1-.45-1-1v-5H6c-.55 0-1-.45-1-1s.45-1 1-1h5V6c0-.55.45-1 1-1s1 .45 1 1v5h5c.55 0 1 .45 1 1s-.45 1-1 1z"></path></svg>
              Review
            </Button>
          </div>
          {reviews.map((item: Reviews, i) =>
            <div className="pt-3" key={i}>
              ...
              <div className="flex items-center text-sm pt-3">
                ...
                <div className="pl-1"> <StarRating rating={Number(item.rating)} /></div>
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <StarRating rating={subRatingVal} onRatingChange={onRatingChange} />
                <div>
                  <Textarea
                    label="Description"
              ...
              <ModalFooter>
                <Button isLoading={subLoading} color="primary" onPress={() => sub()}>
                  OK
                </Button>
              </ModalFooter>
            </>
